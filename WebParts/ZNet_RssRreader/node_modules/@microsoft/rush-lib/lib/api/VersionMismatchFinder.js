...
class VersionMismatchFinder {
    static rushCheck(rushConfiguration) {
        VersionMismatchFinder._checkForInconsistentVersions(rushConfiguration, true);
    }
    static ensureConsistentVersions(rushConfiguration) {
     ...
     */
    static getMismatches(rushConfiguration) {
        // Collect all the preferred versions into a single table
        const allPreferredVersions = {};
        rushConfiguration.commonVersions.getAllPreferredVersions().forEach((version, dependency) => {
        ...
        return new VersionMismatchFinder(projects, rushConfiguration.commonVersions.allowedAlternativeVersions);
    }
    static _checkForInconsistentVersions(rushConfiguration, isRushCheckCommand) {
        if (rushConfiguration.ensureConsistentVersions || isRushCheckCommand) {
            const mismatchFinder = VersionMismatchFinder.getMismatches(rushConfiguration);
                ...
                dependencies.forEach((dependency) => {
                    if (dependency.dependencyType !== "peerDependencies" /* Peer */
                        ...
                        if (!this._mismatches.has(dependency.name)) {
                            this._mismatches.set(dependency.name, new Map());
                        }
                        const dependencyVersions = this._mismatches.get(dependency.name);
