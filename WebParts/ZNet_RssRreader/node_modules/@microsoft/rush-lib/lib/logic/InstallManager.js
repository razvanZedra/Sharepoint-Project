...
class InstallManager {
    ...
    static collectImplicitlyPreferredVersions(rushConfiguration) {
        ...
        rushConfiguration.projects.forEach((project) => {
            InstallManager._collectVersionsForDependencies(versionsForDependencies, project.packageJsonEditor.dependencyList, project.cyclicDependencyProjects, rushConfiguration);
            InstallManager._collectVersionsForDependencies(versionsForDependencies, project.packageJsonEditor.devDependencyList, project.cyclicDependencyProjects, rushConfiguration);
        });
    ...
    constructor(rushConfiguration, purgeManager) {
        ...
        return Promise.resolve().then(() => {
            ...
            return this.ensureLocalPackageManager()
                ...
                if (!options.fullUpgrade) {
                    try {
                        shrinkwrapFile = ShrinkwrapFileFactory_1.ShrinkwrapFileFactory.getShrinkwrapFile(this._rushConfiguration.packageManager, this._rushConfiguration.committedShrinkwrapFilename);
                    }
                    catch (ex) {
                        console.log();
                ...
                const shrinkwrapIsUpToDate = this._createTempModulesAndCheckShrinkwrap(shrinkwrapFile)
    ...
    ensureLocalPackageManager() {
        ...
        const rushUserFolder = this._rushConfiguration.rushUserFolder;
     ...
     */
    _createTempModulesAndCheckShrinkwrap(shrinkwrapFile) {
        ...
        const allExplicitPreferredVersions = this._rushConfiguration.commonVersions
     ...
     */
    _installCommonModules(shrinkwrapIsUpToDate, options) {
        return Promise.resolve().then(() => {
            ...
            if (shrinkwrapIsUpToDate && !deleteNodeModules) {
                ...
                if (this._rushConfiguration.packageManager === 'pnpm') {
                    ...
                    const pnpmFileFilename = path.join(this._rushConfiguration.commonRushConfigFolder, RushConstants_1.RushConstants.pnpmFileFilename);
            ...
            return this._checkIfReleaseIsPublished()
                ...
                if (options.allowShrinkwrapUpdates && !shrinkwrapIsUpToDate) {
                    ...
                    this._syncFile(this._rushConfiguration.tempShrinkwrapFilename, this._rushConfiguration.committedShrinkwrapFilename);
                }
                else {
    ...
    _checkIfReleaseIsPublished() {
        return Promise.resolve().then(() => {
            const lastCheckFile = path.join(this._rushConfiguration.rushUserFolder, 'rush-' + Rush_1.Rush.version, 'last-check.flag');
