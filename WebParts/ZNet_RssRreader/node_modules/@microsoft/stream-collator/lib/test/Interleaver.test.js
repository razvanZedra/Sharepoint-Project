...
Interleaver_1.Interleaver.setStdOut(stdout);
describe('Interleaver tests', () => {
    // Reset task information before each test
    beforeEach(() => {
        Interleaver_1.Interleaver.reset();
        stdout.reset();
    });
    describe('Testing register and close', () => {
        it('cannot be directly instantiated', (done) => {
            chai_1.assert.throws(() => {
                ...
                const a = new Interleaver_1.Interleaver();
                /* tslint:enable:no-unused-variable */
            });
            done();
        });
        it('can register a task', (done) => {
            const helloWorldWriter = Interleaver_1.Interleaver.registerTask('Hello World');
        ...
        it('should not let you register two tasks with the same name', (done) => {
            ...
            chai_1.assert.doesNotThrow(() => { Interleaver_1.Interleaver.registerTask(taskName); });
            chai_1.assert.throws(() => { Interleaver_1.Interleaver.registerTask(taskName); });
            done();
        });
        it('should not let you close a task twice', (done) => {
            const taskName = 'Hello World';
            const task = Interleaver_1.Interleaver.registerTask(taskName);
            task.close();
    ...
    describe('Testing write functions', () => {
        it('writeLine should add a newline', (done) => {
            const taskA = Interleaver_1.Interleaver.registerTask('A');
    ...
    describe('Testing that output is interleaved', () => {
        it('should not write non-active tasks to stdout', (done) => {
            const taskA = Interleaver_1.Interleaver.registerTask('A');
            const taskB = Interleaver_1.Interleaver.registerTask('B');
            taskA.write('1');
        ...
        it('should not write anything when in quiet mode', (done) => {
            const taskA = Interleaver_1.Interleaver.registerTask('A', true);
            const taskB = Interleaver_1.Interleaver.registerTask('B', true);
            taskA.write('1');
